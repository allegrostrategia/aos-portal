...
test("the greeting follows the UK clock through the day", () => {
  ...
  assert.equal(greeting(new Date("2026-09-14T21:00:00Z")), "Buonasera"); // 22:00 UK: still evening, never buonanotte
  assert.equal(greeting(new Date("2026-09-15T03:59:00Z")), "Buonasera"); // 04:59 UK
});

test("it is the UK hour, not the server's", () => {
  assert.equal(greeting(new Date("2026-12-01T23:30:00Z")), "Buonasera");
