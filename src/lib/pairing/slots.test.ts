...
test("the grid is every offered hour on every weekday", () => {
  ...
  assert.equal(grid[0].slots.at(-1), "2026-10-30T20:00");
  assert.equal(grid[0].slots.length, 12);
});
...
test("a slot is one offered hour on one weekday in the month", () => {
  ...
  assert.ok(isSlot("2026-10-06T20:00", "2026-10-01"), "eight in the evening is offered");
  assert.ok(!isSlot("2026-10-06T21:00", "2026-10-01"), "after eight");
