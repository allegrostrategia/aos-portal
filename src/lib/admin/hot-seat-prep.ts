...
export async function getSessionSubmissions(
  ...
  const { data } = await supabase
    ...
    .select(
      "id, member_id, challenge, already_tried, done_looks_like, time_sink, should_stop, reflection, reflection_unsure, submitted_at, suggested_challenge, confirmed_challenge, confirmed_at, attended, members(full_name, email)",
    )
...
export async function getSessionPrep(sessionId: string): Promise<PrepRow[]> {
  ...
  const toRow = (
    ...
    return {
      ...
      timeSink: submission?.time_sink ?? null,
      shouldStop: submission?.should_stop ?? null,
