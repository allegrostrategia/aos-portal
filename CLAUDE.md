...
- **A guard trigger that admits only `is_portal_admin()` refuses the service role too.** The cron and `after()` jobs write with the service role, which has no JWT, so `auth.uid()` is null and `is_portal_admin()` is false. Where the system writes a column a member may not, the guard must say `is_portal_admin() or (select auth.uid()) is null` (as `accrue_hours_for_week` and `guard_pairing_member_update` do), and the service-role write must check its error. The day-7 pairing flag silently never set in production for three weeks this way (state doc, peer pairing section)
